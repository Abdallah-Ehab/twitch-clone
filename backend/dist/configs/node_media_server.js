...
class StreamManager {
    ...
    setupEventHandlers() {
        this.nms.on('ffmpegPublish', (stream) => {
            console.log('[ffmpeg] process started for stream:', stream.streamPath);
            stream.ffmpegProcess?.stderr?.on('data', (data) => {
                console.log('[ffmpeg stderr]', data.toString());
            });
        });
        this.nms.on('prePublish', async (session, streamPath, args) => {
            ...
            try {
                ...
                const ownerName = channel.owner?.username || 'Unknown';
        ...
        this.nms.on('donePublish', async (session, streamPath, args) => {
        ...
        this.nms.on('prePlay', (session, streamPath, args) => {
            console.log('[NodeEvent prePlay]', `id=${session?.id || 'unknown'} streamPath=${streamPath}`);
        });
        this.nms.on('postPlay', (session, streamPath, args) => {
        ...
        this.nms.on('donePlay', (session, streamPath, args) => {
    ...
    viewerCountUpdateTimers = new Map();
    lastViewerCounts = new Map();
    async updateViewerCount(streamKey) {
        const stream = this.activeStreams.get(streamKey);
        if (!stream)
            return;
        const lastCount = this.lastViewerCounts.get(streamKey) ?? -1;
        if (stream.viewerCount === lastCount)
            return;
